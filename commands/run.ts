...
export async function getRunJsonScripts() {
    if (await exists('./run.json')) {
        const runJson = (await readJson('./deno.json')) as runJson;

        return runJson['scripts'] || {};
    }

    if (await exists('./run.yaml')) {
        const runJson = (await parseToYaml()) as runJson;
